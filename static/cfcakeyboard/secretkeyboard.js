...
var publicKey = 'dWKSjDKtw0lr1/If0IZPpUam3MzVIgnBtrhS5RebfZwebwjcbsrU61gFdisSZqsPUNmXX23zhtASDBBIclJQKg=='
var publicKeySig = ''
...
function initInput() {
  ...
  initNumberKeyboard('SIPBox2');
  initSipBoxNum('SIPBox2');
  var keyboard = getKeyboard('SIPBox2');
  if (CFCA_OK != keyboard.setMaxLength(6, 'SIPBox2')) alert('setMaxLength error');
...
function initNumberKeyboard(sipboxId) {
  ...
  var randomType = KEYBOARD_DISORDER_NONE;
...
function initSipBoxNum(sipboxId) {
  ...
  setUpEvent(sipBox, 'focus', function (event) {
    ...
    if (CFCA_OK != numberKeyboard.setServerRandom(serverRandom, sipboxId))
      alert('setServerRandom error');
    if (CFCA_OK != numberKeyboard.setPublicKey(CIPHER_TYPE_RSA, publicKey, publicKeySig, sipboxId))
...
function setProperty(sipboxId) {
  ...
  if (isChangeBackTitle && (sipboxId === 'SIPBox1' || sipboxId === 'SIPBox3'))
    keyboard.setBackBtnTitle(backTitle);
...
function getEncrypt(sipboxId) {
  ...
  var passwordStrengthLevel = keyboard.getWeakCipherInfo(sipboxId);
...
function updateGridValue(element, isEncryptState) {
  ...
  if (isEncryptState || (isEncryptState == undefined)) {
    ...
    for (var i = 0; i < 6; i++) {
      var currentBox = document.getElementById(boxIDs[i]);
      if (i < element.value.length) {
        currentBox.value = element.value.charAt(i);
      } else {
        currentBox.value = '';
      }
    }
...
function showProperty(sipboxId) {
  ...
  var weakCipherInfo = keyboard.getWeakCipherInfo(sipboxId);
